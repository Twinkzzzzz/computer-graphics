...
void Manager::particles_edit_gui(Undo& undo, Scene_Particles& particles) {
    ...
    activate();
    ImGui::RadioButton("Forward Eular", &opt.method, 0);
    activate();
    ImGui::RadioButton("Backward Eular", &opt.method, 1);
    activate();
    ImGui::RadioButton("Symplectic Euler", &opt.method, 2);
    activate();
    ImGui::RadioButton("4-th Runge-Kutta", &opt.method, 3);
    activate();
    ImGui::DragFloat("Weight", &opt.weight, 0.1f, 0.1f, 100.0f, "%.1f");
    activate();
    ImGui::SliderFloat("Damp Coefficient", &opt.dampexp, 0.00f, 1.00f, "%.2f");
    activate();
    ImGui::DragFloat("Froce field", &opt.forceexp, 0.1f, -100.0f, 100.0f, "%.1f");
    activate();
    ImGui::DragFloat("Time Force", &opt.timeforceexp, 0.1f, -50.0f, 50.0f, "%.1f");
    activate();
    ImGui::SliderFloat("Time Force Duration", &opt.timeforcedur, 0.0f, 3.0f, "%.2f");
    activate();
    if(ImGui::Button("Reset Motivation")) {
        opt.velocity = 8.0f;
        opt.angle = 0.0f;
        opt.scale = 0.06f;
        opt.weight = 1.0f;
        opt.dampexp = 0.05f;
        opt.forceexp = 0.0f;
        opt.timeforceexp = 9.8f;
        opt.timeforcedur = 1.0f;
    }
